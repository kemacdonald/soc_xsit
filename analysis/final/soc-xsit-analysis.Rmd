...
### Script to analyze Soc-Xsit experiments
...
# experiment 3
df_expt3 <- read.csv("../../data/soc_xsit_reliability_expt3.csv")
```
...
Flag eposure trials with really slow or fast RTs (+/- 2SD).
...
ggplot(data=ms_expo_rt_expt1, 
       aes(x=intervalNum, y=rt, color = condition)) + 
    geom_pointrange(aes(ymin=rt - ci_low, 
                        ymax=rt + ci_high), width = .05, size=0.6) +
    scale_x_continuous(limits=c(-.9,8), breaks=c(0, 1, 3, 7)) +
...
ggplot(data=ms_test_all_expt1, 
    ...
    scale_y_continuous(limits=c(0,1)) +
    scale_colour_manual(values=c("#1f78b4", "red")) +
    facet_grid(exclusionary_criteria ~ numPic, labeller = mf_labeller) + 
    xlab("Intervening Trials") + 
    ylab("Prop. Choosing Repeated Referent") +
    labs(colour = "Condition") +
    labs(linetype = "Trial Type") +
...
m1_4way_expt1 <- glmer(correct ~ (trialType + condition + 
                           ...
                           (trialType | subids), 
                       data=df_test_expt1_filt, family=binomial, nAGQ=0)
summary(m1_4way_expt1)

# model comparison
anova( m1_2way_expt1, m1_3way_expt1, m1_4way_expt1)
```
...
m1_3way_expt1_unfilt <- glmer(correct ~ (trialType + condition + 
                           ...
                           (trialType | subids), 
                       data=df_test_expt1, family=binomial, nAGQ=0)
summary(m1_3way_expt1_unfilt)

anova(m1_2way_expt1_unfilt, m1_3way_expt1_unfilt)
```
...
ggplot(data=ms_rt_expo_expt2, 
       aes(x=intervalNum, y=rt_exposure, color = condition_trial)) + 
    geom_pointrange(aes(ymin=rt_exposure - ci_low, 
                        ymax=rt_exposure + ci_high), width = .05, size=0.6) +
    scale_x_continuous(limits=c(-.9,4), breaks=c(0,3)) +
    geom_smooth(method="lm", se=F) +
    scale_y_continuous(limits=c(0,5000)) +
    scale_colour_manual(values=c("#1f78b4", "red")) +
...
m1_expt2_3_way_filt <- glmer(correct ~ (trialType + condition_trial + intervalNum)^3 + 
                         ...
                         data = df_test_expt2_filt,
                         family=binomial)
summary(m1_expt2_3_way_filt)

anova(m1_expt2_2way_filt, m1_expt2_3_way_filt)
```
...
m1_expt2_filt <- glmer(correct ~ (trialType + condition_trial + intervalNum)^2 + 
                         ...
                         data = df_test_expt2_filt,
                         family=binomial)
summary(m1_expt2_filt)
```

Model is no different when we use the filtered data.
...
# now plot
ggplot(data=ms_acc_exp_test_expt3, 
