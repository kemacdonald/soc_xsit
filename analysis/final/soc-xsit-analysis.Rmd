...
df_test_expt1 <- df_expt1 %>% filter(trial_category == "test")
...
df_test_expt1 <- df_test_expt1 %>%
    mutate(include_good_rt = ifelse(log(rt) > mean(log(rt)) + 2 * sd(log(rt)) |
                                    ...
                                    "exclude", "include")) %>% 
    filter(include_good_rt == "include")
```
...
df_expo_expt1_analysis <- df_expt1 %>% 
    filter(trial_category == "exposure", rt > 0) %>%
    mutate(include_good_rt = ifelse(log(rt) > mean(log(rt)) + 2 * sd(log(rt)) |
                                        log(rt) < mean(log(rt)) - 2 * sd(log(rt)),
                                    "exclude", "include")) %>%
    filter(include_good_rt == "include")
...
df_test_expt1_filt <- df_test_expt1 %>% 
    filter(include_good_rt == "include", include_expo == "include" | condition == "No-Social") %>%
...
```{r}
df_expt1_analysis_final <- df_test_expt1 %>% 
    dplyr::bind_rows(df_expo_expt1_analysis) %>% 
    dplyr::arrange(subids) 

write.csv(x = df_expt1_analysis_final, 
...
# save output
## ggsave("../../paper/manuscript/figs/exp1-rt-exp.png", exp1_rt_exposure_plot, width=9, height=5)
```
...
# save output
ggsave("../../paper/manuscript/figs/exp1-acc-test.png", expt1.acc.test, width=9, height=5)
```
...
```{r}
df_expt3_analysis_final <- df_test_test %>% 
    dplyr::bind_rows(df_expo_test_expt3) %>% 
    dplyr::arrange(subids) 

write.csv(x = df_expt3_analysis_final, 
