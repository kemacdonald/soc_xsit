...
theme_soc_xsit <- theme_update(
  ...
  axis.title.x = element_text(colour="black",size=18, margin=margin(15,0,0,0),face="plain"),
  ...
  axis.title.y = element_text(colour="black",size=18,margin=margin(0,15,0,0),face="plain"),
  legend.position="top",
...
Clean up the variable names for plotting and plot
...
y_axis_labs <- c("3.0", "4.0", "5.0", "6.0", "7.0")
...
exp1_rt_exposure_plot <- ggplot(data=ms_expo_rt_expt1, 
  ...
  geom_point(color = "white", size = 2.5, 
             data = filter(ms_expo_rt_expt1, condition == "No-Gaze")) +
  scale_y_continuous(limits=c(3,7.5), breaks = c(3,4,5,6,7), labels = y_axis_labs) +
...
#### Now plot inspection time for exposure trials with the accuracy on test trials.

```{r expt2 final plot, echo=F, fig.width = 4, fig.height = 4}
## inspection time plot
ms_rt_expo_expt2$condition <- revalue(ms_rt_expo_expt2$condition_trial,
                                      c("no-social" = "No-Gaze", "social" = "Gaze"))

y_axis_labs_2 <- c("3.0", "4.0", "5.0", "6.0")
...
expt2.rt.expo <- ggplot(data=ms_rt_expo_expt2, 
  ...
  geom_pointrange(aes(ymin=ci_lower, ymax=ci_upper, shape = condition_trial), 
                  fill = "white", stroke = 1.5, size= 1.3, fatten = 3) +
  scale_shape_manual(values = c(21, 19)) +
  scale_y_continuous(limits=c(3,6.5), breaks = c(3,4,5,6), labels = y_axis_labs_2) +
  scale_colour_manual(values=c("#2c7fb8", "#e34a33")) +
  guides(shape=F, color=F) +
  labs(title = "Inspection Time (Exposure)",
       x = "Intervening Trials",
       y = "RT (sec)") +
...
expt2.acc.test.line <- ggplot(data=ms_test_expt2_filt, 
  ...
  geom_line(aes(group=interaction(trialType, condition_trial),
                linetype = trialType), size = 1, position = position_dodge(width = 0.2)) +
  geom_pointrange(aes(ymin=ci_lower, ymax=ci_upper, shape = condition_trial), 
...
grid.arrange(expt2.rt.expo, expt2.acc.test.line, ncol=2)
```
...
### Analyze performance at Test

As a function of reliability condition, inspection time, number of trials using gaze, and subjective reliability judgment.
...
df_inspect <- df_expt3 %>% 
  filter(trial_category == "exposure", include_good_rt == "include") %>% 
...
expt3_gf_plot <- ggplot(aes(x = prop_cond_clean, y = mean, color = correct_exposure), 
                        data = ms_expt3_gf, lineType = correct_exposure) + 
  ...
  geom_pointrange(aes(ymin=ci_lower, ymax=ci_upper, shape = correct_exposure), 
                  fill = "white", stroke = 1.8, size= 1.3, fatten = 1.5,
                  position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.1)) +
  scale_shape_manual(values = c(21, 19)) +
  scale_x_discrete(expand = c(0.2,0)) +
...
e3_main_plot <- grid.arrange(expt3_acc_test, expt3_gf_plot, ncol = 2)

ggsave(plot = e3_main_plot, "../paper/figs/expt3_main_plot.png", device = "png", width = 12, height = 6)
```
...
e3_sub_plots <- grid.arrange(expt3_acc_test_chose_gazetar, expt3_subj_rel_test_plot, 
                             expt3_inspect_plot, ncol = 3)

ggsave(plot = e3_sub_plots, "../paper/figs/expt3_sub_plot.png", device = "png", width = 14, height = 5)
```
