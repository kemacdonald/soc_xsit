...
```{r expt1 accuracy on exposure trials, include=F}
...
```{r expt 1 accuracy on exposure trials plot, include = F, echo=F}
...
expt3_gf_plot <- ggplot(aes(x = prop_cond_clean, y = mean, color = correct_exposure), 
  ...
  labs(color = "Followed Gaze\n(Exposure)",
       ...
       title = "(B)") + 
  guides(shape=F, linetype=F, col = guide_legend(ncol = 2)) +
...
e3_sub_plots <- grid.arrange(expt3_acc_test_chose_gazetar, expt3_subj_rel_test_plot, ncol = 2)
