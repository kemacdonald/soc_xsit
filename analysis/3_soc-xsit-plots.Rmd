...
expt2.acc.test <- ggplot(data=ms_test_expt2_filt, aes(x=as.factor(intervalNum), 
  ...
  geom_line(aes(group=interaction(trialType, condition_trial), linetype = trialType)) +
  geom_pointrange(aes(ymin=ci_lower, ymax=ci_upper, shape = condition_trial), 
...
Get the number of trials on which participants did not make a response before the timer ran out.

```{r}
df_expt4 %>% 
  group_by(answer_type, inspection_cond) %>% 
  summarise(count = n()) 
```
