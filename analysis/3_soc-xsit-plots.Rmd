...
expt1.acc.test <- ggplot(data=ms_test_filt_expt1, aes(x=intervalNum, 
...
df_inspect %<>% mutate(exposure_bin = cut(inspection_time_exposure_sec, 5, 
                                          labels = c("0.0-2.5", "2.5-4.7", "4.7-6.9", 
                                              "6.9-9.1", "9.1-11.4" )))
...
expt3_subj_rel_test_plot <- ggplot(data=ms_rel_subj, 
  ...
  scale_y_continuous(limits=c(0,1)) +
  scale_colour_manual(values=c("#2c7fb8", "#e34a33")) +
  labs(color = "Trial Type", y = NULL, x = "Subjective Reliability") +
  guides(color=FALSE,shape=F) +
...
gridExtra::grid.arrange(expt3_acc_test,
...
```{r}
ggplot(data=ms_e4, aes(x=as.factor(intervalNum), y=mean, 
