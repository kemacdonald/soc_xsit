...
# calculate ages 
demo$birthday = as.Date(demo$birthday, format = "%m/%d/%y")
demo$run_date = as.Date(demo$run_date, format = "%m/%d/%y")
demo$age = (demo$run_date - demo$birthday)/365
demo$age = signif(as.numeric(as.character(demo$age)), 3) # num with 3 digits

# create age groups
demo <- demo %>%
    mutate(age_bin = ifelse(age >= 2 & age < 3, 2, 
                            ifelse(age >= 3 & age < 4, 3, 
                                   ifelse(age >= 4 & age < 5, 4, 
                                          ifelse(age >= 5 & age < 6, 5,
                                                 ifelse(age >= 6 & age < 7, 6, NA))))))
```
...
all.data <- all.data[c("subid","birthday","gender","age","age_bin","date", "condition", 
...
* revalue face vid on exposure trials because the tracking was off in the experiment. faceVid_expo tracks the correct video that was shown during exposure.
...
all.data.2 <- merge(all.data.2, face_vids_exposure, by=c("subid", "itemNum"))
```
...
#reorder columns to put id and demo variables first
all.data.2 <- all.data.2[c("subid","birthday","gender","age","age_bin", "condition", 
                   "trial","itemNum","trialType","samePos","chosen","chosen_idx","kept",
...
```{r save files}
write.csv(d.final.all, file= paste(write_path, "soc-xsit-ipad-alldata-n70.csv", 
                                   sep=""), row.names=F)

write.csv(d.final.test, file = paste(write_path, "soc-xsit-ipad-testdata-n70.csv", 
                                     sep=""), row.names=F)
```