...
```{r read path}
setwd("/Users/kmacdonald/Documents/Projects/SOC_XSIT/processed_data/child/")
```

Read in the data. 

```{r read-data}
data.all_df <- tbl_df(read.csv("soc-xsit-ipad-alldata-n70.csv", sep=","))
data.test_df <- tbl_df(read.csv("soc-xsit-ipad-testdata-n70.csv", sep=","))
```
...
data.test_df <- merge(data.test_df, include_df, by="subid")
```
...
# only kids who got version 2 of the experiment
d.test.filt.v2.old <- filter(data.test_df, include == 1, include_test_trials == 1,
                             condition == "No-social" | correct_exposure == TRUE,
                             version_expt == "v2", 
                             age_bin >= 4)
```
...
ms_subs_v2 <- d.test.filt.v2 %>%
    group_by(subid, condition, trial_type_redo, age_bin) %>%
    summarise(mean_correct = mean(correct),
              ci_high = ci.high(correct), 
              ci_low = ci.low(correct), 
              n_trial = n())
...
Plot accuracy on test trials for each age bin.

```{r plot acc for all ages}
ms_v2_age_bins <- d.test.filt.v2.old %>%
    group_by(condition, trial_type_redo, age_bin) %>%
    summarise(mean_correct = mean(correct),
              ci_high = ci.high(correct), 
              ci_low = ci.low(correct), 
              n_trial = n(),
...
ggplot(data = filter(ms_v2_age_bins, age_bin %in% c(4,5)), 
       aes(x = condition, y = mean_correct, 
           group = trial_type_redo, colour = trial_type_redo)) +
    geom_errorbar(aes(ymin = mean_correct - ci_low, 
                      ymax = mean_correct + ci_high), width = 0.05) +
