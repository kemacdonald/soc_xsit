...
Now vizualize accuracy on test trials separated by whether subjects chose target of eye gaze

```{r choose eye gaze analysis}
...
m1.b <- glmer(correct ~ trialType + prop_cond_clean +
                (trialType | subids),
            data = df_test_test, 
            family = binomial)
summary(m1.b)
...
m2.noslope <- glmer(correct ~ trialType * reliability +
                (1 | subids),
            data = df_test_test, 
            family = binomial)
summary(m2.noslope)
...
m2.lm <- lm(correct ~ trialType * reliability,
            data = mss)
summary(m2.lm)

# reliablity as continuous 
m2.b <- glmer(correct ~ trialType + reliability +
                (trialType | subids),
            data = df_test_test, 
            family = binomial)
summary(m2.b)
...
m3.b <- glmer(correct ~ poly(reliability, 2) + trialType +
                (trialType | subids),
             data = df_test_test, nACQ=0,
            family = binomial)
summary(m3.b)
...
```{r model with accuracy on exposure as predictor}
df_test_test <- df_test_test %>%
    group_by(subids) %>% 
    mutate(total_exposure_correct = sum(correct_exposure)) 

m4 <- glmer(correct ~ total_exposure_correct * trialType +
                (trialType | subids),
             data = df_test_test, nACQ=0,
            family = binomial)
summary(m4)
...
m4.b <- glmer(correct_exposure ~ prop_cond_clean +
                (1 | subids),
             data = df_test_test, nACQ=0,
            family = binomial)
summary(m4.b)
...
qplot(total_exposure_correct, correct, col=trialType, 
      ymin=correct-cil, ymax = correct+cih, geom="pointrange",
      data=ms) + geom_smooth(method="lm")