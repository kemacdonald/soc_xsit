...
%DIF DEL soc-xsit-elsevier_pre.tex   Mon Sep 14 13:43:09 2015
%DIF ADD soc-xsit-elsevier.tex       Tue Sep 15 18:38:36 2015
...
\title{\DIFdelbegin \DIFdel{Distributional or discrete? }\DIFdelend Social cues modulate the representations underlying cross-situational
learning}
...
\begin{abstract}
\DIFdelbegin \DIFdel{Word learning }\DIFdelend \DIFaddbegin \DIFadd{Because children hear language in environments that contain many things
to talk about, learning the meaning of even the simplest word }\DIFaddend requires
making inferences \DIFdelbegin \DIFdel{from noisy data -- even
concrete nouns occur in contexts with many possible referents. A }\DIFdelend \DIFaddbegin \DIFadd{under undertainty. A cross-situational }\DIFaddend statistical
learner can \DIFdelbegin \DIFdel{handle this ambiguity by aggregating }\DIFdelend \DIFaddbegin \DIFadd{aggregate }\DIFaddend across naming events to form stable \DIFdelbegin \DIFdel{word-object mappings. But each naming event
...
representation used for statistical learning}\DIFdelend \DIFaddbegin \DIFadd{word-referent
mappings, but this approach neglects an important source of information
that can reduce referential uncertainty: social cues from speakers}\DIFaddend . In
three large-scale experiments with adults, we test the \DIFdelbegin \DIFdel{effect }\DIFdelend \DIFaddbegin \DIFadd{effects }\DIFaddend of
varying referential uncertainty \DIFdelbegin \DIFdel{on attention and memory during }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend cross-situational word learning \DIFdelbegin \DIFdel{. Referential cues shift }\DIFdelend \DIFaddbegin \DIFadd{using
social cues. Social cues shifted }\DIFaddend learners away from tracking multiple
hypotheses \DIFaddbegin \DIFadd{and }\DIFaddend towards storing only a single hypothesis (Experiments 1
and 2). In addition, learners \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend sensitive to graded changes in the
strength of a \DIFdelbegin \DIFdel{referential }\DIFdelend \DIFaddbegin \DIFadd{social }\DIFaddend cue, and when it \DIFdelbegin \DIFdel{becomes }\DIFdelend \DIFaddbegin \DIFadd{became }\DIFaddend less reliable, \DIFdelbegin \DIFdel{learners are }\DIFdelend \DIFaddbegin \DIFadd{they were
}\DIFaddend more likely to store multiple hypotheses (Experiment 3). \DIFdelbegin \DIFdel{Together, the }\DIFdelend \DIFaddbegin \DIFadd{Our }\DIFaddend data
suggest that \DIFdelbegin \DIFdel{representations in }\DIFdelend \DIFaddbegin \DIFadd{the representations underlying }\DIFaddend cross-situational word
learning are quite flexible: In conditions of greater uncertainty,
learners tend to store a broader range of information.
\end{abstract}

\begin{keyword}
statistical learning, \DIFdelbegin \DIFdel{pragmatic }\DIFdelend \DIFaddbegin \DIFadd{social }\DIFaddend cues, word learning, language acquisition
\end{keyword}
...
approximate the input distribution (for \DIFdelbegin \DIFdel{a }\DIFdelend review, see Smith, Suanda, \& Yu
2014). One approach is to model learning as \DIFdelbegin \DIFdel{updates to }\DIFdelend \DIFaddbegin \DIFadd{a process of updating
}\DIFaddend connection strengths between multiple word-object links (McMurray,
Horst, \& Samuelson, 2012), while other approaches argue that learners
\DIFdelbegin \DIFdel{only store }\DIFdelend \DIFaddbegin \DIFadd{store only }\DIFaddend a single word-object link (Trueswell, Medina, Hafri, \&
...
\DIFdelbegin \DIFdel{Another ongoing debate in the literature is }\DIFdelend \DIFaddbegin \DIFadd{In addition to the debate about representation, researchers also
disagree about }\DIFaddend how to best characterize the \DIFaddbegin \DIFadd{ambiguity of the }\DIFaddend input to
cross-situational learning mechanisms. One way researchers have
quantified \DIFaddbegin \DIFadd{this }\DIFaddend ambiguity is to ask adults to guess the meaning of an
...
\DIFdelbegin \DIFdel{pragmatic }\DIFdelend \DIFaddbegin \DIFadd{social }\DIFaddend cue, a speaker's gaze, changes the representations underlying
cross-situational word learning. We use a modified version of Yurovsky
\& Frank (2015)'s paradigm, which \DIFdelbegin \DIFdel{provides }\DIFdelend \DIFaddbegin \DIFadd{we describe in greater depth below, to
provide }\DIFaddend a direct measure of memory for alternative word-object links
...
representations underlying cross-situational word learning. \DIFdelbegin \DIFdel{Participants
saw a
series of exposure trials where we manipulated }\DIFdelend \DIFaddbegin \DIFadd{We use a
version of Yurovsky }\& \DIFadd{Frank (2015)'s paradigm where we manipulate }\DIFaddend the
ambiguity of the learning context by including a gaze cue from a
schematic, female interlocutor. \DIFdelbegin \DIFdel{On each exposure trial, participants }\DIFdelend \DIFaddbegin \DIFadd{Participants saw a series of ambiguous
exposure trials where they }\DIFaddend heard one novel word that was either paired
...
using a more ecologically valid stimulus set. \DIFdelbegin \DIFdel{To move closer to a real
world referential cue, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend replaced the static,
schematic drawing with a \DIFaddbegin \DIFadd{video of a }\DIFaddend live actress. \DIFaddbegin \DIFadd{While the video
...
object. }\DIFaddend To reduce the across-conditions variability, we introduced a
...
our next experiment, we test a \DIFdelbegin \DIFdel{clear }\DIFdelend prediction of our account: whether
learners \DIFdelbegin \DIFdel{tendency to store multiple }\DIFdelend \DIFaddbegin \DIFadd{store more }\DIFaddend word-object links \DIFdelbegin \DIFdel{responds }\DIFdelend \DIFaddbegin \DIFadd{in response }\DIFaddend to graded changes of
...
cue strength by including a block of familiarization trials \DIFdelbegin \DIFdel{at the start
of the experiment where we
establish the reliability of }\DIFdelend \DIFaddbegin \DIFadd{where we
vary the proportion of Same and Switch trials. If participants saw more
Switch trials, this provides evidence that }\DIFaddend the speaker's gaze \DIFaddbegin \DIFadd{was an
unreliable cue to reference}\DIFaddend . This design was inspired by a growing body
...
The critical contribution of Experiment 3 is \DIFdelbegin \DIFdel{the finding }\DIFdelend that learners respond to a
\emph{graded} manipulation of referential uncertainty, with the amount
of information stored from the intital exposure tracking with \DIFdelbegin \DIFdel{both }\DIFdelend the
reliability of the cue and participants' use of the cue. This graded
accuracy performance \DIFdelbegin \DIFdel{provides support for a critical prediction
of our account: that learners store a single candidate word meaning and
a set of alternatives }\DIFdelend \DIFaddbegin \DIFadd{shows that at the group-level learners stored
alternative word-object links }\DIFaddend with different levels of fidelity
...
\DIFaddend Why did we not see an increase in the strength of learners' candidate
...
rather than with the globally optimal strategy. \DIFdelbegin \DIFdel{Here, we
show evidence
that learners adapt their allocation of cognitive resources to the level
}\DIFdelend \DIFaddbegin \DIFadd{In the current work, we
found that learners showed evidence of changing how they allocated
cognitive resources based on the amount }\DIFaddend of referential uncertainty\DIFdelbegin \DIFdel{in the learning context}\DIFdelend ,
spending less time studying alternative word-object links and reducing
the number of links stored in memory when uncertainty \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend low.
...
the important point that only the \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{information }\DIFaddend that comes into contact
with the learning system can be used for cross-situational word
learning, and this \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{information }\DIFaddend is directly influenced by the attention
...
\DIFadd{We are grateful to Rose Schneider for helping record stimuli and to the
...
to DY, and a John Merck Scholars Fellowship to M.C.F}\DIFaddend .
