...
abstract:
    "Because children hear language in environments that contain many things to talk about, learning the meaning of even the simplest word requires making inferences under uncertainty. A cross-situational statistical learner can aggregate across naming events to form stable word-referent mappings, but this approach neglects an important source of information that can reduce referential uncertainty: social cues from speakers (e.g., eye gaze). In four large-scale experiments with adults, we test the effects of varying referential uncertainty in cross-situational word learning using social cues. Social cues shifted learners away from tracking multiple hypotheses and towards storing only a single hypothesis (Experiments 1 and 2). In addition, learners were sensitive to graded changes in the strength of a social cue, and when it became less reliable, they were more likely to store multiple hypotheses (Experiment 3). Finally, learners were more likely to store a single hypothesis in the presence of a social cue even if inspection time was fixed across the social and non-social conditions (Experiment 4). Our data suggest that the representations underlying cross-situational word learning are quite flexible: In conditions of greater uncertainty, learners tend to store a broader range of information."
...
nsubs_expt4 <- df_expt4 %>%
  group_by(social_cond, interval, inspection_cond) %>%
  distinct(subid) %>% 
  summarise(n_subs = n())
```
...
Participants were randomly assigned to one of the 32 between-subjects conditions (4 Referents X 4 Intervals X 2 Gaze). Participants either saw 2, 4, 6, or 8 referents on the screen and test trials occurred at different intervals after exposure trials: either 0, 1, 3, or 7 trials from the initial exposure to a word. For example, in the 0-interval condition the test trial for that word would occur immediately following the exposure trial, but in the 3-interval condition participants saw three other exposure trials before getting the test trial for that word.
...
```{r expt1-plot, fig.pos = "tb", fig.width=4.8, fig.height=6, fig.cap = "Experiment 1 results. Panel A shows response times on exposure trials across all experimental conditions: Gaze and No-gaze, Referents (2, 4, 6, and 8), and Intervening trials (0, 1, 3, and 7). Panel B shows accuracy on test trials for Same and Switch trials across all conditions. The horizontal dashed lines represent chance performance for each condition. Colored lines are linear model fits and error bars indicate 95\\% confidence intervals computed by non-parametric bootstrap."}
...
m1_rt_expt1 <- lmer(rt ~ condition * log2(intervalNum + 1) * log2(numPicN) + (1|subid), 
...
beta_inspect_expt1 <- round(summary(m1_2way_acc_expt1_inspect)$coef[13], 2)
p_inspect_expt1 <- round(summary(m1_2way_acc_expt1_inspect)$coef[13,4], 3)

# now fit the 3way model
m1_3way_acc_expt1_filt <- glmer(correct ~ (trialType + condition + 
                                             log2(intervalNum + 1) +
                                             log2(numPicN)+
                                             log2(inspection_time_exposure_sec))^3 +
                                  (trialType | subid), offset = logit(1/numPicN),
                                control=glmerControl(optimizer="bobyqa"),
                                family=binomial, 
                                nAGQ=0, 
...
```{r expt3 test performance gaze-follow trial-level, include = F}
ms_expt3_gf <- df_expt3 %>% 
  filter(trial_category == "test", include_good_rt == "include",
         block == "test") %>% 
  group_by(subids, trialType, correct_exposure, reliability) %>% 
  summarise(mean_ss = mean(correct)) %>%
  group_by(trialType, correct_exposure, reliability) %>%
  multi_boot_standard(column = "mean_ss")


ggplot(aes(x = reliability, y = mean, color = correct_exposure), 
       data = ms_expt3_gf) + 
  geom_pointrange(aes(ymax = ci_upper, ymin = ci_lower), position = position_jitter(width=0.1)) +
  geom_line(aes(group=interaction(trialType, correct_exposure), linetype = trialType)) +
...
```{r expt4 exclusions}
n_hits <- 400
expt4_nsubs_excluded = n_hits - sum(nsubs_expt4$n_subs)
```
...
```{r expt4 chance exposure}
e3.chance.tests.exposure <- test.chance(data = filter(df_expt4, trial_category == "exposure", 
                                                      include_good_rt == "include", 
                                                      gaze_trial == "Gaze"),
...
```{r expt4 lmer rt}
m1_rt_expt3 <- lmer(rt ~ condition_trial * log2(intervalNum + 1) + (1|subids), 
                    data=filter(df_expt2, trial_cat == "exposure"))
...
```{r expt4 test chance}
e4.chance.tests <- test.chance(data = filter(df_expt4, trial_category == "test"),
...
```{r expt4 glmer}
m1_acc_expt4 <- glmer(correct ~ (trialType + gaze_trial + log2(intervalNum + 1))^2 +
                        (trialType | subid), 
                      nAGQ=0,
                      glmerControl(optimizer = "bobyqa"),
                      family=binomial,
