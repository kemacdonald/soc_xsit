...
hypothesis in the presence of a social cue even if inspection time was
fixed across the social and non-social conditions (Experiment 4). Our
data suggest that the representations underlying cross-situational word
...
if participants did not respond correctly on familiar object trials (165
...
condition the test trial for that word would occur immediately following
the exposure trial, but in the 3-interval condition participants saw
three other exposure trials before getting the test trial for that word.
...
compared participant's accuracy on Exposure trials in the Gaze condition
to a model of random behavior defined as a Binomial distribution with a
probability of success \(\frac{1}{Num Referents}\). Correct performance
is defined as selecting the object that was the target of the speaker's
gaze. Following Yurovsky \& Frank (2015), we fit logistic regressions
for each Gaze, Referent, and Interval combination specified as
\texttt{Gaze Target $\sim$ 1 + offset(logit(1/Referents))}. The offset
encodes the chance probability of success given the number of referents,
and the coefficient for the intercept term shows on a log-odds scale how
much more likely participants are to select the gaze target than would
...
than expected by chance (Overall mean = 0.83, smallest \(\beta\) = 2.41,
z = 10.26, p \textless{} .001).
...
We found a significant main effect of referents (\(\beta\) = 728.22, p
...
and number of referents (\(\beta\) = -500.3, p \textless{} .001) such
...
\(\beta\) = 0.3, z = 1.88, p = 0.06). On Switch trials in the 8
referent, 3 interval condition, participants' responses were not
significantly different from chance (\(\beta\) = 0.08, z = 0.45, p =
0.65). Participants' success on Switch trials replicates the findings
...
\begin{tabular}{lrrrrl}
 Predictor & Estimate & Std. Error & $z$ value & $p$ value &  \\ 
  \hline
Intercept & 3.17 & 0.31 & 10.25 & $<$ .001 & *** \\ 
  Switch Trial & -1.52 & 0.25 & -6.12 & $<$ .001 & *** \\ 
  Gaze Condition & -0.30 & 0.27 & -1.11 & 0.27 &  \\ 
  Log(Interval) & -0.50 & 0.12 & -4.30 & $<$ .001 & *** \\ 
  Log(Referents) & 0.22 & 0.12 & 1.90 & 0.06 & . \\ 
  Switch Trial*Gaze Condition & -0.88 & 0.12 & -7.05 & $<$ .001 & *** \\ 
  Switch Trial*Log(Interval) & 0.58 & 0.06 & 10.14 & $<$ .001 & *** \\ 
  Switch Trial*Log(Referent) & -0.59 & 0.09 & -6.56 & $<$ .001 & *** \\ 
  Gaze Condition*Log(Interval) & 0.02 & 0.06 & 0.34 & 0.73 &  \\ 
  Gaze Condition*Log(Referent) & 0.28 & 0.10 & 2.93 & 0 & ** \\ 
  Log(Interval)*Log(Referent) & -0.05 & 0.04 & -1.21 & 0.23 &  \\ 
   \hline
\end{tabular}
...
significant main effects of Referents (\(\beta = 0.22\), p \textless{}
.001) and Interval (\(\beta = -0.5\), p \textless{} .001), such that as
each of these factors increased, accuracy on test trials decreased. We
also found significant main effects of Trial Type (\(\beta = -1.52\), p
...
(\(\beta = 0.58\), p \textless{} .001), Trial Type and Referents
(\(\beta = -0.59\), p \textless{} .001), and Gaze condition and
Referents (\(\beta = 0.28\), p \textless{} .05). These interactions can
...
Trial Type and Gaze condition (\(\beta = -0.88\), p \textless{} .001),
...
400 paid HITs (excluded 78 HITs).
...
for total of 400 paid HITs (excluded 35 HITs).
...
block consisted of 8 exposure trials and 8 test trials (4 Same trials
and 4 Switch trials), and contained only Gaze or No-gaze exposure
trials. The order of block was counterbalanced across participants. The
major design change was to equate the length of exposure trials in the
Gaze and No-Gaze conditions, allowing us to test for the effect of
