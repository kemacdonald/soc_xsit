...
Hafri, \& Gleitman, 2013). Recent experimental and modeling work,
Yurovsky and Frank (2015) suggests an integrative explanation: learners
...
Yurovsky, Smith, and Yu (2013) found a more bimodal distribution, with
approximately 20\% of naming events being ambiguous (\textless{} 10\%
accuracy) and 30\% being unambiguous (\textgreater{} 90\% accuracy).
Cartmill, Armstrong, Gleitman, Goldin-Meadow, Medina, and Trueswell
(2013) also showed that the proportion of unambiguous naming episodes
varies across parent-child dyads, with some parents rarely providing
highly informative contexts and others' doing so relatively more
often.\footnote{The differences in the estimates of referential
...
that young learners tended to retain labels that were accompanied with
clear referential cues that were concurrent with visual access. And
correlational data has demonstrated strong links between early
...
learners tracking of alternatives but apparently did not cause learners
...
schematic drawing with a video of a live actress. While these stimuli
...
0.58, z = 9.68, \(p\) \textless{} .001). We replicated the critical
...
word-object links (\(\beta = -0.65\), \(p\) \textless{}
...
and test increased (\(\beta\) = -0.98, \(p\) \textless{} .001) and on
the Switch trials overall (\(\beta = -3.04\), \(p\) \textless{} .001).

In addition, there was a significant interaction between trial type and
interval (\(\beta = 0.85\), \(p\) \textless{} .001), with worse
...
significant (\(\beta = 0.14\), \(p\) = 0.057), such that participants in
...
them towards single hypothesis tracking. Moving to a live action
...
50\%, 75\%, and 100\% reliability conditions. After the familiarization
...
number of alternative word-object links, and not learners' candidate
hypothesis. However, when the length of exposure trials was equivalent
...
Why else would a decrease in memory for alternatives fail to increase
the strength of learners' memory for their candidate hypothesis? One
possibility is that people did not shift their cognitive resources from
the set of alternatives to their single hypothesis, but instead chose to
conserve their resources for future use. Griffiths, Lieder, and Goodman
...
the nature of the input. Our findings suggest that referential cues
interact with statistical learning by modulating the amount of
...
Moreover, previous work with infants has shown that their attention is
...
referential cues. Finally, we presented novel words in isolation,
