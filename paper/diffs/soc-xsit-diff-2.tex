...
%DIF DEL soc-xsit-elsevier_revision_draft1.tex   Mon Sep 12 17:21:17 2016
%DIF ADD soc-xsit-elsevier.tex                   Fri Sep 16 14:49:08 2016
...
Yurovsky, Smith, and Yu (2013) found \DIFdelbegin \DIFdel{more of a bimodal distribution of referential uncertainty}\DIFdelend \DIFaddbegin \DIFadd{a higher proprotion of clear naming
...
shorter in the Gaze (\(\beta\) = -1.11, \DIFdelbegin \DIFdel{p }\DIFdelend \DIFaddbegin \DIFadd{\(p\) }\DIFaddend \textless{} .001) and the
3-interval condition (\(\beta\) = -0.5, \DIFdelbegin \DIFdel{p }\DIFdelend \DIFaddbegin \DIFadd{\(p\) }\DIFaddend \textless{} .001). The
...
not significant. These analyses provide evidence that \DIFdelbegin \emph{\DIFdel{use}} %DIFAUXCMD
...
total of 400 paid HITs (\DIFdelbegin \DIFdel{excluded }\DIFdelend 37 HITs \DIFaddbegin \DIFadd{excluded}\DIFaddend ).
...
hypothesis in the gaze condition\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend an effect that we did not \DIFdelbegin \DIFdel{find }\DIFdelend \DIFaddbegin \DIFadd{see }\DIFaddend in
Experiments 1-3. One explanation for this difference is that in
Experiment 4\DIFaddbegin \DIFadd{, since }\DIFaddend participants' use of gaze was independent of the
length of exposure trials, \DIFdelbegin \DIFdel{which made }\DIFdelend inspection times in the gaze condition \DIFaddbegin \DIFadd{were
}\DIFaddend longer compared to those in Experiments 1-3. Thus, it could be that the
combination of \DIFdelbegin \DIFdel{gaze with longer inspection times leads to }\DIFdelend \DIFaddbegin \DIFadd{a gaze cue with the opportunity to continue attending to
the gaze target led to }\DIFaddend a boost in performance on Same trials relative to
\DIFdelbegin \DIFdel{learning }\DIFdelend \DIFaddbegin \DIFadd{trials }\DIFaddend without gaze.
...
result in a \DIFaddbegin \DIFadd{strong }\DIFaddend boost to memory for learners' candidate hypothesis.
This pattern of data suggests that the presence of a referential cue
selectively affected one component of the underlying representation: the
number of alternative word-object links, and not learners' \DIFdelbegin \DIFdel{initial
}\DIFdelend candidate
hypothesis. However, when the length of exposure trials was \DIFdelbegin \DIFdel{made }\DIFdelend equivalent
across the gaze \DIFaddbegin \DIFadd{and no-gaze }\DIFaddend conditions in Experiment 4, learners showed
stronger memory for their initial hypothesis when gaze was present\DIFdelbegin \DIFdel{. This }\DIFdelend \DIFaddbegin \DIFadd{,
suggesting that the confound with inspection time might have masked this
difference in the previous experiments. This finding in turn }\DIFaddend suggests
that the relationship between referential cues and the strength of
learners' candidate hypothesis is modulated by how the cue interacts
with \DIFdelbegin \DIFdel{visual inspection time: when coupled with longer
inspection times}\DIFdelend \DIFaddbegin \DIFadd{attention: When coupled with the opportunity for sustained
attention}\DIFaddend , gaze provided a boost to memory.

\subsection{Relationship to previous
work}\label{relationship-to-previous-work}

Why \DIFdelbegin \DIFdel{did we not see an increase in }\DIFdelend \DIFaddbegin \DIFadd{would a decrease in memory for alternatives fail to increase }\DIFaddend the
strength of learners' \DIFaddbegin \DIFadd{memory for their }\DIFaddend candidate hypothesis in
Experiments 1-3? One possibility is that participants did not shift
their cognitive resources from the set of alternatives to their single
hypothesis, but instead \DIFdelbegin \DIFdel{rationally conserved }\DIFdelend \DIFaddbegin \DIFadd{chose to use the gaze information to reduce
inspection time, thus conserving }\DIFaddend their resources for future use.
Griffiths, Lieder, and Goodman (2015) formalize this behavior by pushing
the rationality of computational-level models down to the psychological
process level. In their framework, cognitive systems are thought to be
adaptive in that they optimize the use of their limited resources,
taking the cost of computation (e.g., the opportunity cost of time or
mental \DIFdelbegin \DIFdel{opportunity}\DIFdelend \DIFaddbegin \DIFadd{energy}\DIFaddend ) into account. For example, Vul, Goodman, Griffiths, and
Tenenbaum (2014) showed that as time pressure increased in a
decision-making task, participants were more likely to show behavior
...
found that learners showed evidence of altering how they allocated
cognitive resources based on the amount of referential uncertainty
present during learning, spending less time inspecting alternative
word-object links and reducing the number of links stored in memory when
uncertainty was low.
...
interaction between statistical learning \DIFaddbegin \DIFadd{mechansims }\DIFaddend and other types of
...
people store in the underlying representations that support learning
over time.
}
...
}\DIFdelend \DIFaddbegin \DIFadd{more }\DIFaddend difficult. Thus, we think that it will be important
to test a variety of outcome measures to see if \DIFdelbegin \DIFdel{young }\DIFdelend \DIFaddbegin \DIFadd{younger }\DIFaddend learners show
evidence of storing multiple word meanings during learning.
\DIFaddbegin 

\DIFaddend Moreover, previous work with infants has shown that their attention is
often stimulus-driven and sticky (Oakes, 2011), suggesting that very
...
information modulates the input to statistical learning mechanisms}\DIFdelend \DIFaddbegin \DIFadd{statistical learning mechansims operate over fundamentally social input}\DIFaddend .
