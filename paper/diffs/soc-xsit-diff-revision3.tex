...
%DIF DEL soc-xsit-elsevier-revision-draft2.tex   Thu Sep 22 12:03:16 2016
%DIF ADD soc-xsit-elsevier-revision-draft3.tex   Fri Sep 30 13:44:16 2016
...
The key behavioral prediction of our hypothesis \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend that the presence of
...
\DIFadd{In the main text, we only report effects that achieved statistical
...
the paper. }\DIFaddend All models were fit using the lme4 package in R (Bates,
...
\texttt{Correct $\sim$ \DIFaddbegin \DIFadd{(}\DIFaddend Trial Type \DIFdelbegin \DIFdel{* }\DIFdelend \DIFaddbegin \DIFadd{+ }\DIFaddend Gaze + \DIFdelbegin \DIFdel{Trial Type * }\DIFdelend Log(Interval) + \DIFdelbegin \DIFdel{Trial Type * }%DIFDELCMD < \\ %%%
...
shorter \DIFdelbegin \DIFdel{in the Gaze }\DIFdelend \DIFaddbegin \DIFadd{when gaze was present }\DIFaddend (\(\beta\) = -1.1, \(p\) \textless{} .001)
and \DIFaddbegin \DIFadd{in }\DIFaddend the 3-interval condition (\(\beta\) = -0.48, \(p\) \textless{}
...
\(p\) \textless{} .001). \DIFdelbegin \DIFdel{However, we }\DIFdelend \DIFaddbegin \DIFadd{We also }\DIFaddend found an interaction between inspection
time and trial type (\(\beta\) = 0.21, \(p\) = 0.05), with longer
inspection times providing a larger boost to performance on Switch
trials \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(i.e., stronger memory for alternative word-object links). }\DIFaddend This
result differs slightly from \DIFdelbegin \DIFdel{what we found in
}\DIFdelend Experiment 1 where \DIFaddbegin \DIFadd{we found an interaction
between trial type and inspection time, with }\DIFaddend longer inspection times
\DIFdelbegin \DIFdel{led to better }\DIFdelend \DIFaddbegin \DIFadd{providing a larger boost to }\DIFaddend accuracy in the No-Gaze condition. \DIFdelbegin \DIFdel{It seems plausible that more time spent visually
exploring the objects during learning would lead }\DIFdelend \DIFaddbegin \DIFadd{Despite
...
}\DIFaddend to better performance on Switch trials \DIFdelbegin \DIFdel{, which depend }\DIFdelend \DIFaddbegin \DIFadd{since these trials depended }\DIFaddend on
encoding multiple \DIFdelbegin \DIFdel{alternatives. Thus,
the
interaction between inspection time and gaze condition }\DIFdelend \DIFaddbegin \DIFadd{word-object links. It is also possible that the
interaction betweeen gaze condition and inspection time that we }\DIFaddend found in
Experiment 1 \DIFdelbegin \DIFdel{might have been driven by the fact that longer inspection
times were more likely to occur in the absence of a gaze cue}\DIFdelend \DIFaddbegin \DIFadd{was influenced by the different number of referents and
interval conditions}\DIFaddend .
...
it. The interaction between trial type and reliability \DIFdelbegin \DIFdel{became marginally
}\DIFdelend \DIFaddbegin \DIFadd{was no longer
}\DIFaddend significant in this model (\(\beta = -0.62\), \(p\) = 0.086), suggesting
that \DIFdelbegin \DIFdel{participants' use of the gaze cue was a stronger predictor of }\DIFdelend \DIFaddbegin \DIFadd{the effect of reliability on learners' }\DIFaddend memory for alternative
word-object links \DIFaddbegin \DIFadd{is driven by whether participants chose to use the
gaze cue during learning}\DIFaddend .\footnote{We are grateful to an anonymous
...
trials. \DIFdelbegin \DIFdel{There was a marginally significant }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend interaction between inspection time and reliability
condition \DIFaddbegin \DIFadd{was not significant. The key interaction between reliability
condition and trial type remained significant in this version of the
model }\DIFaddend (\(\beta\) = \DIFdelbegin \DIFdel{-0.2}\DIFdelend \DIFaddbegin \DIFadd{-0.58}\DIFaddend , \(p\) = \DIFdelbegin \DIFdel{0.067)with longer inspection times providing a larger boost to accuracy
when the speaker was less reliable}\DIFdelend \DIFaddbegin \DIFadd{0.048)}\DIFaddend .

Next, we explored the factors that influenced inspection time on
exposure trials by \DIFdelbegin \DIFdel{predicting inspection times using }\DIFdelend \DIFaddbegin \DIFadd{fitting a model to predict inspection times as a
function of }\DIFaddend reliability condition and participants' use of the gaze cue\DIFdelbegin \DIFdel{as predictors}\DIFdelend .
...
objects \DIFdelbegin \DIFdel{on exposure trials}\DIFdelend \DIFaddbegin \DIFadd{during learning}\DIFaddend .
...
\section*{\DIFadd{Table A1. Length of inspection times on exposure trials in Experiment 1 as a function of gaze, interval, and number of referents}}

\texttt{\DIFadd{Log(Inspection time) $\sim$ (Gaze + Log(Interval) + Log(Referents))$^2$ + (1 | subject)}}

\begin{table}[h]
\centering
\begin{tabular}{lrrrrl}
\DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{t.value }& \DIFaddFL{p.value }&  \\ 
\hline
\DIFaddFL{Intercept }& \DIFaddFL{0.83 }& \DIFaddFL{0.10 }& \DIFaddFL{8.19 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Gaze Condition }& \DIFaddFL{0.16 }& \DIFaddFL{0.11 }& \DIFaddFL{1.48 }& \DIFaddFL{0.138 }&  \\ 
\DIFaddFL{Log(Interval) }& \DIFaddFL{0.06 }& \DIFaddFL{0.05 }& \DIFaddFL{1.33 }& \DIFaddFL{0.184 }&  \\ 
\DIFaddFL{Log(Referents) }& \DIFaddFL{0.34 }& \DIFaddFL{0.04 }& \DIFaddFL{7.91 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Gaze Condition*Log(Interval) }& \DIFaddFL{-0.08 }& \DIFaddFL{0.03 }& \DIFaddFL{-2.86 }& \DIFaddFL{0.004 }& \DIFaddFL{** }\\ 
\DIFaddFL{Gaze Condition*Log(Referent) }& \DIFaddFL{-0.27 }& \DIFaddFL{0.04 }& \DIFaddFL{-6.01 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Log(Interval)*Log(Referent) }& \DIFaddFL{-0.00 }& \DIFaddFL{0.02 }& \DIFaddFL{-0.19 }& \DIFaddFL{0.849 }&  \\ 
\hline
\end{tabular}
\label{tab:e1_rt}
\end{table}
...
\section*{\DIFadd{Table A2. Accuracy on test trials in Experiment 1 with inspection times on exposure trials included as a predictor}}

\texttt{\DIFadd{Correct $\sim$ (Trial Type + Gaze + Log(Interval) + Log(Referents) + }\\ \DIFadd{Log(Inspection Time))$^2$ + offset(logit($^1/_{Referents}$)) + (TrialType | subject)}}

\begin{table}[h]
\centering
\begin{tabular}{lrrrrl}
\DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{z.value }& \DIFaddFL{p.value }&  \\ 
\hline
\DIFaddFL{Intercept }& \DIFaddFL{2.89 }& \DIFaddFL{0.34 }& \DIFaddFL{8.49 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Switch Trial }& \DIFaddFL{-1.45 }& \DIFaddFL{0.25 }& \DIFaddFL{-5.76 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Gaze Condition }& \DIFaddFL{0.12 }& \DIFaddFL{0.27 }& \DIFaddFL{0.43 }& \DIFaddFL{0.669 }&  \\ 
\DIFaddFL{Log(Interval) }& \DIFaddFL{-0.47 }& \DIFaddFL{0.11 }& \DIFaddFL{-4.15 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Log(Referents) }& \DIFaddFL{0.05 }& \DIFaddFL{0.14 }& \DIFaddFL{0.39 }& \DIFaddFL{0.693 }&  \\ 
\DIFaddFL{Log(Inspection Time) }& \DIFaddFL{0.20 }& \DIFaddFL{0.15 }& \DIFaddFL{1.38 }& \DIFaddFL{0.169 }&  \\ 
\DIFaddFL{Switch Trial*Gaze Condition }& \DIFaddFL{-1.02 }& \DIFaddFL{0.13 }& \DIFaddFL{-7.86 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Switch Trial*Log(Interval) }& \DIFaddFL{0.52 }& \DIFaddFL{0.06 }& \DIFaddFL{9.39 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Switch Trial*Log(Referent) }& \DIFaddFL{-0.62 }& \DIFaddFL{0.09 }& \DIFaddFL{-6.67 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Switch Trial*Log(Inspection Time) }& \DIFaddFL{0.09 }& \DIFaddFL{0.07 }& \DIFaddFL{1.36 }& \DIFaddFL{0.174 }&  \\ 
\DIFaddFL{Gaze Condition*Log(Interval) }& \DIFaddFL{0.09 }& \DIFaddFL{0.06 }& \DIFaddFL{1.61 }& \DIFaddFL{0.107 }&  \\ 
\DIFaddFL{Gaze Condition*Log(Referent) }& \DIFaddFL{0.36 }& \DIFaddFL{0.10 }& \DIFaddFL{3.68 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Gaze Condition*Log(Inspection Time) }& \DIFaddFL{-0.17 }& \DIFaddFL{0.07 }& \DIFaddFL{-2.55 }& \DIFaddFL{0.011 }& \DIFaddFL{* }\\ 
\DIFaddFL{Log(Interval)*Log(Referent) }& \DIFaddFL{-0.05 }& \DIFaddFL{0.04 }& \DIFaddFL{-1.26 }& \DIFaddFL{0.207 }&  \\ 
\DIFaddFL{Log(Interval)*Log(Inspection Time) }& \DIFaddFL{0.02 }& \DIFaddFL{0.03 }& \DIFaddFL{0.54 }& \DIFaddFL{0.589 }&  \\ 
\DIFaddFL{Log(Referents)*Log(Inspection Time) }& \DIFaddFL{0.05 }& \DIFaddFL{0.05 }& \DIFaddFL{0.94 }& \DIFaddFL{0.345 }&  \\ 
\hline
\end{tabular}
\label{tab:e1_acc_it}
\end{table}
...
\section*{\DIFadd{Table A3. Length of inspection times on exposure trials in Experiment 2 as a function of gaze and interval}}

\texttt{\DIFadd{Log(Inspection time) $\sim$ Gaze * Log(Interval) + (1 | subject)}}
...
\DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{t.value }& \DIFaddFL{p.value }&  \\ 
\hline
\DIFaddFL{Intercept }& \DIFaddFL{3.90 }& \DIFaddFL{0.08 }& \DIFaddFL{50.69 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Gaze Condition }& \DIFaddFL{-1.10 }& \DIFaddFL{0.05 }& \DIFaddFL{-20.90 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Log(Interval) }& \DIFaddFL{-0.48 }& \DIFaddFL{0.05 }& \DIFaddFL{-8.77 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Gaze Condition*Log(Interval) }& \DIFaddFL{-0.02 }& \DIFaddFL{0.04 }& \DIFaddFL{-0.60 }& \DIFaddFL{0.549 }&  \\ 
\hline
\end{tabular}
\label{tab:e2_rt}
\end{table}

\section*{\DIFadd{Table A4. Accuracy on test trials in Experiment 2 with inspection times on exposure trials included as a predictor}}

\texttt{\DIFadd{Correct $\sim$ (Trial Type + Gaze + Log(Interval) + Log(Inspection Time))$^2$ + }\\ \DIFadd{offset(logit($^1/_{Referents}$)) + (TrialType | subject)}}
...
\DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{z.value }& \DIFaddFL{p.value }&  \\ 
\hline
\DIFaddFL{Intercept }& \DIFaddFL{2.41 }& \DIFaddFL{0.29 }& \DIFaddFL{8.34 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Gaze Condition }& \DIFaddFL{0.13 }& \DIFaddFL{0.23 }& \DIFaddFL{0.58 }& \DIFaddFL{0.559 }&  \\ 
\DIFaddFL{Switch Trial }& \DIFaddFL{-3.12 }& \DIFaddFL{0.26 }& \DIFaddFL{-12.21 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Log(Interval) }& \DIFaddFL{-0.88 }& \DIFaddFL{0.14 }& \DIFaddFL{-6.34 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Log(Inspection Time) }& \DIFaddFL{0.15 }& \DIFaddFL{0.13 }& \DIFaddFL{1.14 }& \DIFaddFL{0.255 }&  \\ 
\DIFaddFL{Switch Trial*Gaze Condition }& \DIFaddFL{-0.54 }& \DIFaddFL{0.17 }& \DIFaddFL{-3.21 }& \DIFaddFL{0.001 }& \DIFaddFL{** }\\ 
\DIFaddFL{Gaze Condition*Log(Interval) }& \DIFaddFL{0.16 }& \DIFaddFL{0.09 }& \DIFaddFL{1.85 }& \DIFaddFL{0.064 }& \DIFaddFL{. }\\ 
\DIFaddFL{Gaze Condition*Log(Inspection Time) }& \DIFaddFL{-0.14 }& \DIFaddFL{0.10 }& \DIFaddFL{-1.37 }& \DIFaddFL{0.172 }&  \\ 
\DIFaddFL{Switch Trial*Log(Interval) }& \DIFaddFL{0.77 }& \DIFaddFL{0.10 }& \DIFaddFL{8.00 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Switch Trial*Log(Inspection Time) }& \DIFaddFL{0.21 }& \DIFaddFL{0.11 }& \DIFaddFL{1.96 }& \DIFaddFL{0.05 }& \DIFaddFL{. }\\ 
\DIFaddFL{Log(Interval)*Log(Inspection Time) }& \DIFaddFL{0.04 }& \DIFaddFL{0.06 }& \DIFaddFL{0.77 }& \DIFaddFL{0.44 }&  \\ 
\hline
\end{tabular}
\label{tab:e2_acc_it}
\end{table}
...
\section*{\DIFadd{Table A5. Accuracy on exposure trials in Experiment 3 as a function of reliability condition and participants' subjective reliability judgments}}

  \texttt{\DIFadd{Correct-Exposure $\sim$ Reliability Condition * Subjective Reliability + (1 | subject)}}
  ...
  \DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{z.value }& \DIFaddFL{p.value }&  \\ 
  \hline
  \DIFaddFL{Intercept }& \DIFaddFL{1.97 }& \DIFaddFL{0.98 }& \DIFaddFL{2.02 }& \DIFaddFL{0.044 }& \DIFaddFL{* }\\ 
  \DIFaddFL{Reliability Condition }& \DIFaddFL{3.28 }& \DIFaddFL{1.50 }& \DIFaddFL{2.19 }& \DIFaddFL{0.029 }& \DIFaddFL{* }\\ 
  \DIFaddFL{Subjective Reliability }& \DIFaddFL{7.26 }& \DIFaddFL{1.73 }& \DIFaddFL{4.21 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
  \DIFaddFL{Reliability Condition*Subjective Reliability }& \DIFaddFL{-4.58 }& \DIFaddFL{2.72 }& \DIFaddFL{-1.68 }& \DIFaddFL{0.092 }& \DIFaddFL{. }\\ 
  \hline
  \end{tabular}
  \label{tab:e3_gf_exp}
  \end{table}

  \section*{\DIFadd{Table A6. Accuracy on test trials in Experiment 3 as a function of reliability condition}}

  \texttt{\DIFadd{Correct $\sim$ Trial Type * Reliability Condition + (Trial Type | subject)}}
  ...
  \DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{z.value }& \DIFaddFL{p.value }&  \\ 
  \hline
  \DIFaddFL{Intercept }& \DIFaddFL{3.61 }& \DIFaddFL{0.36 }& \DIFaddFL{10.04 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
  \DIFaddFL{Trial Type }& \DIFaddFL{-3.95 }& \DIFaddFL{0.36 }& \DIFaddFL{-10.93 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
  \DIFaddFL{Reliability Condition }& \DIFaddFL{0.38 }& \DIFaddFL{0.37 }& \DIFaddFL{1.03 }& \DIFaddFL{0.302 }&  \\ 
  \DIFaddFL{Reliability Condition*Trial Type }& \DIFaddFL{-0.76 }& \DIFaddFL{0.38 }& \DIFaddFL{-2.01 }& \DIFaddFL{0.044 }& \DIFaddFL{* }\\ 
  \hline
  \end{tabular}
  ...
  \section*{\DIFadd{Table A7. Accuracy on test trials in Experiment 3 as a function of reliability condition and participants' use of gaze on exposure trials}}

\texttt{\DIFadd{Correct $\sim$ (Trial Type + Reliability Condition + Correct-Exposure)$^2$ }\\ \DIFadd{+ (Trial Type | subject)}}
...
\DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{z.value }& \DIFaddFL{p.value }&  \\ 
\hline
\DIFaddFL{Intercept }& \DIFaddFL{3.40 }& \DIFaddFL{0.39 }& \DIFaddFL{8.75 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Correct Exposure }& \DIFaddFL{0.07 }& \DIFaddFL{0.29 }& \DIFaddFL{0.26 }& \DIFaddFL{0.797 }&  \\ 
\DIFaddFL{Trial Type }& \DIFaddFL{-2.70 }& \DIFaddFL{0.38 }& \DIFaddFL{-7.06 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Reliability Condition }& \DIFaddFL{-0.43 }& \DIFaddFL{0.44 }& \DIFaddFL{-0.98 }& \DIFaddFL{0.325 }&  \\ 
\DIFaddFL{Correct Exposure*Trial Type }& \DIFaddFL{-1.43 }& \DIFaddFL{0.27 }& \DIFaddFL{-5.41 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Correct Exposure*Reliability }& \DIFaddFL{0.97 }& \DIFaddFL{0.33 }& \DIFaddFL{2.92 }& \DIFaddFL{0.004 }& \DIFaddFL{** }\\ 
\DIFaddFL{Reliability Condition*Trial Type }& \DIFaddFL{-0.62 }& \DIFaddFL{0.36 }& \DIFaddFL{-1.72 }& \DIFaddFL{0.086 }& \DIFaddFL{. }\\ 
\hline
\end{tabular}
\label{tab:e3_acc_rel_cond_gf}
\end{table}

\section*{\DIFadd{Table A8. Accuracy on test trials in Experiment 3 as a function of each participants' accuracy on exposure trials}}

  \texttt{\DIFadd{Correct $\sim$ Trial Type * Total Correct Exposure + (Trial Type | subject)}}
  ...
  \DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{z.value }& \DIFaddFL{p.value }&  \\ 
  \hline
  \DIFaddFL{Intercept }& \DIFaddFL{2.38 }& \DIFaddFL{0.54 }& \DIFaddFL{4.39 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
  \DIFaddFL{Total Exposure Correct }& \DIFaddFL{0.19 }& \DIFaddFL{0.07 }& \DIFaddFL{2.65 }& \DIFaddFL{0.008 }& \DIFaddFL{** }\\ 
  \DIFaddFL{Trial Type }& \DIFaddFL{-2.11 }& \DIFaddFL{0.54 }& \DIFaddFL{-3.89 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
  \DIFaddFL{Total Exposure Correct*Trial Type }& \DIFaddFL{-0.32 }& \DIFaddFL{0.07 }& \DIFaddFL{-4.45 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
  \hline
  \end{tabular}
  ...
  \section*{\DIFadd{Table A9. Accuracy on test trials in Experiment 3 as a function of each participants' subjective reliability judgment}}

\texttt{\DIFadd{Correct $\sim$ Trial Type * Subjective Reliability + (Trial Type | subject)}}
...
\DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{z.value }& \DIFaddFL{p.value }&  \\ 
\hline
\DIFaddFL{Intercept }& \DIFaddFL{3.44 }& \DIFaddFL{0.44 }& \DIFaddFL{7.83 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Subjective Reliability }& \DIFaddFL{0.40 }& \DIFaddFL{0.58 }& \DIFaddFL{0.69 }& \DIFaddFL{0.493 }&  \\ 
\DIFaddFL{Trial Type }& \DIFaddFL{-3.44 }& \DIFaddFL{0.44 }& \DIFaddFL{-7.81 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Subjective Reliability*Trial Type }& \DIFaddFL{-1.63 }& \DIFaddFL{0.59 }& \DIFaddFL{-2.78 }& \DIFaddFL{0.005 }& \DIFaddFL{** }\\ 
\hline
\end{tabular}
\label{tab:e3_acc_subj_rel}
\end{table}

\section*{\DIFadd{Table A10. Accuracy on test trials in Experiment 3 as a function of reliability condition and inspection time on exposure trials}}

\texttt{\DIFadd{Correct $\sim$ (Trial Type + Reliability condition + Trial Type + }\\ \DIFadd{Log(Inspection Time))$^2$ + (Trial Type | subject)}}
...
\DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{z.value }& \DIFaddFL{p.value }&  \\ 
\hline
\DIFaddFL{Intercept }& \DIFaddFL{2.01 }& \DIFaddFL{0.20 }& \DIFaddFL{10.31 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Log(Inspection Time) }& \DIFaddFL{0.31 }& \DIFaddFL{0.09 }& \DIFaddFL{3.31 }& \DIFaddFL{0.001 }& \DIFaddFL{** }\\ 
\DIFaddFL{Trial Type }& \DIFaddFL{-2.75 }& \DIFaddFL{0.20 }& \DIFaddFL{-13.64 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Reliability Condition }& \DIFaddFL{0.50 }& \DIFaddFL{0.30 }& \DIFaddFL{1.66 }& \DIFaddFL{0.097 }& \DIFaddFL{. }\\ 
\DIFaddFL{Log(Inspection Time)*Trial Type }& \DIFaddFL{0.03 }& \DIFaddFL{0.09 }& \DIFaddFL{0.34 }& \DIFaddFL{0.736 }&  \\ 
\DIFaddFL{Log(Inspection Time)*Reliability Condition }& \DIFaddFL{-0.20 }& \DIFaddFL{0.11 }& \DIFaddFL{-1.83 }& \DIFaddFL{0.067 }& \DIFaddFL{. }\\ 
\DIFaddFL{Trial Type*Reliability Condition }& \DIFaddFL{-0.58 }& \DIFaddFL{0.29 }& \DIFaddFL{-1.97 }& \DIFaddFL{0.048 }& \DIFaddFL{* }\\ 
\hline
\end{tabular}
...
\section*{\DIFadd{Table A11. Accuracy on test trials in Experiment 4 as a function of gaze and interval}}

\texttt{\DIFadd{Correct $\sim$ (Trial Type + Gaze + Log(Interval))$^2$ + (Trial Type | subject)}}
...
\DIFaddFL{term }& \DIFaddFL{estimate }& \DIFaddFL{std.error }& \DIFaddFL{z.value }& \DIFaddFL{p.value }&  \\ 
\hline
\DIFaddFL{Intercept }& \DIFaddFL{2.27 }& \DIFaddFL{0.16 }& \DIFaddFL{14.37 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Trial Type }& \DIFaddFL{-3.18 }& \DIFaddFL{0.16 }& \DIFaddFL{-19.93 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Gaze Condition }& \DIFaddFL{-0.48 }& \DIFaddFL{0.14 }& \DIFaddFL{-3.52 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Log(Interval) }& \DIFaddFL{-0.84 }& \DIFaddFL{0.10 }& \DIFaddFL{-8.59 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Trial Type*Gaze Condition }& \DIFaddFL{0.90 }& \DIFaddFL{0.14 }& \DIFaddFL{6.63 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Trial Type*Log(Interval) }& \DIFaddFL{0.80 }& \DIFaddFL{0.09 }& \DIFaddFL{8.71 }& \DIFaddFL{$<$ .001 }& \DIFaddFL{*** }\\ 
\DIFaddFL{Gaze Condition*Log(Interval) }& \DIFaddFL{-0.01 }& \DIFaddFL{0.07 }& \DIFaddFL{-0.10 }& \DIFaddFL{0.917 }&  \\ 
\hline
\end{tabular}
\label{tab:e4_acc}
\end{table}
